...
def read_convertion_body(header, raw, curline):
	global prd_convertion
	product = raw[header['product']].lower()
	convert_to = raw[header['convert_to']].lower()
	prd_convertion[product] = convert_to

def read_convertion(product_convertion_filename):
	prd_convertion = {}
	global prd_convertion
	# read convertion
	read_file(product_convertion_filename, empty_header, read_convertion_body)
	# make structure
	convert_final = {}
	for product in prd_convertion:
		next = prd_convertion[product]
		while prd_convertion.has_key(next):
			next = prd_convertion[next]
		convert_final[product] = next
	prd_convertion = None

	return convert_final
